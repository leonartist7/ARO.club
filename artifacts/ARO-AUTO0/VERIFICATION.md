...
## AUTO0 report-contract correction — September 8, 2026

Report imports now require bundle-root run.json matching the dispatcher's repository, revision, audit-only authority and complete normalized source fingerprints. Missing, duplicate or mismatched inputs stop import before evidence is copied. Hashes establish consistency, not independent truth. C1 writes and imports use C1/<AUDIT_SHA>/; exported bundles place report.json and run.json together at their root, so compare --previous points to that extracted bundle. Browser evidence runs on every main push, including dependency and build configuration changes. Local verification: 18 tooling tests passed. Product implementation remains disabled.
