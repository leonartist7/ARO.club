...
Version 1.0.0 · 2026-09-22 · **SPEC-READY — bounded test-only implementation; separate exclusive claim required**

Controller-persisted test-only contract under the founder's readiness execution instruction. No candidate code commit exists at specification acceptance. It permits only the bounded test implementation after the exclusive claim; provider/runtime changes remain prohibited. The one CI attempt requires independent finished-diff review.
...
| V1-03 | Capture the original committed approval tuple and side-effect counts. Repeat the existing complete save-review then approval operation. Require no duplicate teacher, verification, public decision or approval-transition audit; require the original committed decision and its side effects to remain unchanged. Record subsequent mutable review field, actor and timestamp changes separately. Do not fail solely because a later review timestamp differs from the original terminal decision. Escalate unauthorized writes, changed committed outcomes, duplicated effects or a specifically identified violation of the approved contract. |
...
Repeated approval may expose a specifically identified approved-contract violation even when duplicate counts pass. Preserve such evidence and stop for a separate repair decision; later mutable-review timestamp differences alone are observations, not failures. Never add an immutability guard in test-only work. Likewise, duplicate submitted-state rejection can satisfy the no-side-effects oracle without inventing idempotent success semantics.
...
- Controller accepted two extra reviewers, 5→7→0, fixed commands, original duplicate side-effect semantics and race consistency. These test-scope decisions are accepted with the independent V1-03 clarification below; execution still requires an exclusive claim.
...
Controller accepts this bounded verification contract under the founder's 2026-09-22 readiness execution instruction. Independent nonwriting reviewer /root/ready1_trust_review validated runner order, fixed fixture boundaries, exact5→7→0, source preservation and disclosed deferrals; required V1-03 clarification is incorporated verbatim above. No policy change, new terminal-review immutability, hosted authority, expired-JWT coverage or full I02-08 acceptance is granted. READY1 references are pinned to PR62 head0e935e1f2a53d3edc7fca3dbe21feef5ab9d032d, not assumed present in main. Current source base remains f37dc084d7172415f581a41e90d2edd9ba3738b9. Historic preparation-only sentences describe the prior proposal; this acceptance record supersedes only their proposed status. Stop if new source drift changes scope. Writer must not push/trigger CI until independent finished-test-diff review. Controller will persist exact claim and verify readback before dispatch.
