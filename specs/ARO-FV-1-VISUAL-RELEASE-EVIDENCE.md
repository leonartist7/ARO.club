...
- Status: **SPEC-READY — F1–F6 are accepted predecessors; F7 is BLOCKED pending the v0.2.3 documentation merge and exact bundled Chromium launch; release approval WITHHELD**.
- Version: **0.2.3**, founder approved 2026-09-14. User-visible behavior and F1–F7 scope are unchanged from v0.2.2. This narrowly permits Intel Xeon Platinum **8272CL** for F7 measurement only, while preserving every other §20 condition and threshold. Owner/director: ARO founder/product-design.
- Historical approvals: the v0.2.0 candidate was `a576640cc5b9828486d8bdd0f970636b3ff07138`; the v0.2.1 encoder amendment and v0.2.2 640,000-byte persona exception remain preserved. The post-merge v0.2.3 F7 binding is defined in section 25; no earlier documentation or implementation PR is an active F7 execution lane.
- Authoritative FV-1 specification: this file. The recovered v0.1.0 proposal and superseded v0.2.0–v0.2.2 execution wording are historical inputs except where this edition explicitly preserves a limit or accepted predecessor.
...
- Exclusive F7 execution lane: `codex/fv1-f7-acceptance-evidence`, PR #48. Its immutable predecessor is accepted F6 `79603ae1af60a30f86c105e0f2a4d841043eb727` (`F7_TASK_BASE_SHA`). It contains the bounded preflight record only; no F7 functional or controlled-performance measurement has run. Preserve this branch and do not substitute a historical PR or create a different F7 branch.
...
| FV1-07 | F7 integrated performance/visual/accessibility | profile fingerprint, every sample, traces/screenshots, manual screen-reader result | BLOCKED — v0.2.3 documentation merge and exact bundled Chromium revision 1234 install + headless launch are required before measurement |
| FV1-08 | F7 quality/scope/independent review | command exits, required CI, full allowlist/network diff, independent reviewed SHA/verdict and closed findings | BLOCKED — pending FV1-07; no F7 runner, quality gate or independent review has started |
...
**Three separate permissions:** documentation merge publishes the reviewed v0.2.3 specification and factual recovery record; implementation approval authorizes only the already accepted F1–F6 history plus F7 work within this edition; release approval authorizes an eventual product merge/deployment. None implies the others. Release remains withheld.

**F7 post-merge binding:** this documentation-only PR #47 starts from current `main` reconciliation commit `f37dc084d7172415f581a41e90d2edd9ba3738b9` and preserves its N1/MERGE1 work. Let **C** be the actual full `main` merge commit created when PR #47 normally merges. Resolve C from GitHub only after that merge exists. Then `F7_APPROVED_SPEC_SHA = C`; verify that `git show C:specs/ARO-FV-1-VISUAL-RELEASE-EVIDENCE.md` is v0.2.3, records the founder-approved 8272CL-only amendment, names PR #48 as the exclusive F7 lane, and retains all §20 thresholds and human gates. C is the approved F7 specification binding, not a replacement for the accepted predecessor.

`F7_TASK_BASE_SHA` remains the immutable accepted F6 commit `79603ae1af60a30f86c105e0f2a4d841043eb727`. After C exists, resume only the existing `codex/fv1-f7-acceptance-evidence` branch/PR #48; preserve its preflight evidence, revalidate ancestry and clean checkout, and perform the bounded compatibility check against the integrated N1/MERGE1 runtime before any F7 measurement. Do not reapply historical patches, use floating main/HEAD, or create a different F7 branch.

No F7 controlled functional/performance measurement may begin until C exists and the exact lockfile-managed Chromium 151.0.7922.34 / revision 1234 installs in the external Playwright cache, its executable SHA-256 is recorded, and a headless launch is recorded. This does not permit a system browser, relax a budget, replace the three-run method, or waive independent review, human NVDA/Windows testing, founder visual review, product merge, deployment, promotion or release.
...
- Documentation: PR #47 is the documentation-only v0.2.3 merge gate. Its actual full main merge commit C becomes `F7_APPROVED_SPEC_SHA` as section 25 defines.
- Implementation: F1–F6 are accepted predecessors; the existing F7-only lane is PR #48 on `codex/fv1-f7-acceptance-evidence` with immutable `F7_TASK_BASE_SHA` F6 `79603ae1af60a30f86c105e0f2a4d841043eb727`. It may resume only after C and the exact bundled-browser launch record exist.
...
Package FV-1; v0.2.3 is founder-approved but pending normal documentation-only merge through PR #47. The actual merge commit C becomes `F7_APPROVED_SPEC_SHA`; F7's immutable accepted predecessor remains F6 `79603ae1af60a30f86c105e0f2a4d841043eb727`. Existing F7 branch/PR #48 has completed only the bounded preflight: the 8272CL CPU requires this amendment and exact lockfile-managed Chromium revision 1234 was absent/unlaunchable. No F7 functional or controlled performance measurement, quality-gate run, independent review, human NVDA test or founder visual review has run. All thresholds and release gates remain WITHHELD. Recovery provenance remains in [RECOVERY.md](../docs/fv1-recovery/RECOVERY.md).