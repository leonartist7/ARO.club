...
- Status: **SPEC-READY — F1–F6 are accepted predecessors; F7 is founder-authorized but BLOCKED pending the v0.2.4 documentation merge C, reviewed PR #48 runtime reconciliation, and exact bundled Chromium preflight; release approval WITHHELD**.
- Version: **0.2.4**, founder approved 2026-09-24. User-visible behavior and F1–F7 scope are unchanged from v0.2.3. This corrects the F7 production-build launch and adds a fail-closed isolated Next.js environment contract; it does not relax a host, browser, privacy, network, performance, image, accessibility, human-review or release gate. Owner/director: ARO founder/product-design.
- Historical approvals: the v0.2.0 candidate was `a576640cc5b9828486d8bdd0f970636b3ff07138`; the v0.2.1 encoder amendment, v0.2.2 640,000-byte persona exception and v0.2.3 8272CL-only F7 host amendment remain preserved. Founder approval of this v0.2.4 amendment is recorded in `docs/autonomous-control-20260916/FOUNDER-F7-APPROVAL-20260924.md` at controller commit `c145ebd52395a9e8a0104cdaa5ddd78a4672ff35`.
- Authoritative FV-1 specification: this file. The recovered v0.1.0 proposal and superseded v0.2.0–v0.2.3 execution wording are historical inputs except where this edition explicitly preserves a limit or accepted predecessor.
...
- Exclusive F7 execution lane: `codex/fv1-f7-acceptance-evidence`, PR #48. Its immutable historical predecessor is accepted F6 `79603ae1af60a30f86c105e0f2a4d841043eb727` (`F7_TASK_BASE_SHA`). The actual tested Next execution head must be a separately reviewed descendant after C; F6 is never rewritten or presented as a Next runtime. Preserve this branch and do not substitute a historical PR or create a different F7 branch.
- Depends on existing UX0–UX3 static prototype and recovered A1/A2/A3/A4/S1 evidence. Implementation approval and this amendment approval are recorded in section 28; dispatch requires the merged revision binding in section 25; does not unlock I0/P1 or later runtime packages.
...
- **F7 lab-profile amendment, founder approved 2026-09-14:** for F7 measurement only, Intel Xeon Platinum **8272CL** is permitted in place of the 8573C model above. Every other condition in this section remains unchanged, including the 8-CPU-equivalent quota, 20 GiB memory, 9 affinity CPUs, Node/npm, lockfile-pinned Playwright **1.62.0**, exact Chromium **151.0.7922.34** revision **1234**, browser executable SHA-256 and launch record, DPR/theme/viewport settings, 4× CPU throttle, network emulation, three-run/10-second method, all image/byte/performance/accessibility ceilings, independent review and release WITHHELD. This amendment becomes operative only after the normal documentation-only merge of PR #47 containing v0.2.4; it never permits a system-browser substitute.

- F1 encoder subprofile: CPython **3.12.13** remains valid with Pillow **12.3.0** and libwebp **1.6.0**. CPython **3.13.5** is also permitted, but only with Pillow **12.3.0** and libwebp **1.6.0**, and only when the exact F1 synthetic preflight is rerun in the implementation environment and recorded as F1 evidence showing **88 bytes**, SHA-256 `a2dd5e25c6a9dd61b8194f496c8b3995a5ba2efc0abc9079ecd2b6cd8ec5450b`, three byte-identical encodes, and exact RGBA decode including alpha preservation. The prior F1 evidence record reproduced those exact results under CPython 3.13.5. Any version or fingerprint mismatch remains environment/profile drift and is a stop condition. This alternate interpreter allowance does not weaken any image byte budget, real-image crop/alpha check, manifest reproducibility requirement, encoder parameter, or other acceptance/stop condition.
- Capability inventory on 2026-09-08 in existing cloud task `6a9fbdd1-0584-83e8-89e5-27bb2d3e32db`, turn `70c70f6d-4305-4154-aaf3-6a8b9989081f`, found repository Playwright installed but its Chromium executable absent. The founder now authorizes provisioning only the existing lockfile-managed Chromium with `node node_modules/playwright/cli.js install chromium` after C, reviewed PR #48 runtime reconciliation and the isolated lab preflight. This is test-runtime setup, no new package/lockfile dependency or infrastructure. It has NOT been performed. If download, executable hash or headless launch fails, F7 stops; this is an execution capability gap, not a human-only test.
- **F7 isolated Next build-and-runtime environment contract, founder approved 2026-09-24:** this applies only after C exists and the separately reviewed §25 PR #48 compatibility reconciliation establishes the exact tested Next execution head. C must retain these current-main input bytes: `package.json` Git blob `1ab15b62c73d9c62dc9d75ef8890c2ccf2aad3c0`, 1,733 bytes, SHA-256 `a8ccb13a2639b5995087b293dd3fde1fc671caec0e219bce0e9f488ef5348fa2`; `package-lock.json` Git blob `8869494acba733e8f62b558f261c8934f10f6c63`, 300,788 bytes, SHA-256 `95d47a55db74e44756d95d74c682dff6fec9d03d35eecb1649c68bb853913b96`; `next.config.ts` Git blob `9e2b68bb3325a89addde5202680bdad6a157af0a`, 270 bytes, SHA-256 `52c254a9bada836e8f12d4fb506cc16405a035ca3121526170325c68808293df`; and tracked `src/config/ux0.js` blob `71008f72ea1c4af01d9b2d01040239aa017a6872` with `UX0_PROTOTYPE_MODE = true`. SHA-256 values are computed from the Git blob bytes, independent of working-tree line-ending conversion. Any mismatch is a stop condition; never substitute a command, dependency, browser, port or configuration.
- Use a new clean checkout and separate external evidence root. Reject root `.env*` files except `.env.example`/`.env.sample`, `.vercel/`, inherited build output, a symlinked `.next`, an occupied port, and any non-empty inherited application, provider, auth, analytics, AI, payment, deployment, proxy, dotenv, browser-override or Node-injection setting. The build/server child environment retains only necessary OS path/temp/locale variables and sets `NODE_ENV=production`, `CI=true`, `NEXT_TELEMETRY_DISABLED=1`, both `NEXT_PUBLIC_ENABLE_*_ACCOUNTS=false`, and all public Supabase, production-reference and Vercel values empty. Record variable names and empty/false attestations only—never values. Use the same sanitized environment for build and runtime; remove only a validated, untracked, non-symlinked `.next` directory. `npm ci` restores the bound lockfile and is setup, not acceptance evidence.
- Build with `npm run build`. Serve only the resulting production artifact with `npm run preview -- --hostname 127.0.0.1`, which at the bound bytes invokes `next start -p 5173 --hostname 127.0.0.1`. F7’s sole local base URL is `http://127.0.0.1:5173`. Vite-only `--host`/`--strictPort`, the development server, hosted production, another port and a system browser are prohibited. Build/listener failure, a changed script, or an external request is a stop condition.
- AUTO0’s reviewed Next sanitizer (fresh `.next`, clean child environment, disabled accounts, empty provider configuration and loopback process) is a reference for isolation mechanics only. Its port 5199, source inventory/route-handler discovery, 150-observation matrix, forced-theme capture, Google-font allowance and unthrottled observations are not F7 evidence and do not extend the F7 route set or change its network/budget rules. F7 stays limited to the §23 route set and local static content; unexpected external requests fail. Cache remains disabled, service workers blocked, CPU throttling remains 4×, network remains 150 ms / 200,000 bytes/s down / 93,750 bytes/s up, and the three-cold-run/10-second method and every frozen ceiling remain unchanged.
...
Run each named packet test, then npm run lint -- --max-warnings=0, npm test -- --run, npm run build and existing required CI on the finished diff. F7 implements node e2e/fv1.mjs --base http://127.0.0.1:5173 --out ABSOLUTE_EXTERNAL_EVIDENCE_ROOT using the profile above. The runner is a future deliverable, not a test claimed executed now. Data/RLS/migration/provider tests are N/A to this frontend-only delta; existing required CI remains required.

## 24. Acceptance and evidence

| ID | Owner and requirement | Required evidence | Status |
|---|---|---|---|
| FV1-01 | F1 finite reproducible media and stable accessible component | 9 unchanged source hashes, 23 derivative hashes/bytes/dimensions/parameters, repeat --check, component tests, alpha/crop comparisons | ACCEPTED PREDECESSOR — not rerun by F7 |
| FV1-02 | F2 shell ownership/exits/recovery/focus | route table, focused tests, keyboard/focus path, targets and screenshots | ACCEPTED PREDECESSOR — not rerun by F7 |
| FV1-03 | F3 numeric consistency and truthful preview | three fixtures, threshold/capacity/reset/double-click/direct/invalid cases, network observations | ACCEPTED PREDECESSOR — not rerun by F7 |
| FV1-04 | F4 truthful readable discovery/Create | exact link targets, composited contrast, cold media selections/crops | ACCEPTED PREDECESSOR — not rerun by F7 |
| FV1-05 | F5 Profile/Express accessibility | all nodes/choices, five widths/two themes, keyboard/200% zoom/contrast/target evidence | ACCEPTED PREDECESSOR — not rerun by F7 |
| FV1-06 | F6 truthful return/Library/Settings | six-row disposition, claims/copy, no self-links, media measurements | ACCEPTED PREDECESSOR — F6 is `F7_TASK_BASE_SHA`; not rerun by F7 |
| FV1-07 | F7 integrated performance/visual/accessibility | profile fingerprint, every sample, traces/screenshots, manual screen-reader result | BLOCKED — v0.2.4 documentation merge C, reviewed PR #48 Next-runtime reconciliation, and exact bundled Chromium revision 1234 install + headless launch are required before measurement |
| FV1-08 | F7 quality/scope/independent review | command exits, required CI, full allowlist/network diff, independent reviewed SHA/verdict and closed findings | BLOCKED — pending FV1-07; no F7 runner, quality gate or final independent review has started |

F1–F6 predecessor evidence remains preserved. F7 records its full base/spec/tested commit, commands/exits and retrievable evidence references only in existing PR #48 and its tracked F7 evidence files. F7 consolidates `artifacts/ARO-FV-1/VERIFICATION.md` and `acceptance.json`; large captures attach to PR #48 or its GitHub Actions artifact run with hashes and retention/expiry. A local temporary path alone does not pass retrieval. Preserve original failure evidence. No new evidence service or planning system.

## 25. Approval, base reconciliation and rollout

**Three separate permissions:** documentation merge publishes the reviewed v0.2.4 specification and factual recovery record; implementation approval authorizes only the already accepted F1–F6 history plus F7 work within this edition; release approval authorizes an eventual product merge/deployment. None implies the others. Release remains withheld.

**F7 post-merge binding:** this documentation-only PR #47 starts from current `main` reconciliation commit `f37dc084d7172415f581a41e90d2edd9ba3738b9` and preserves its N1/MERGE1 work. Let **C** be the actual full `main` merge commit created when PR #47 normally merges. Resolve C from GitHub only after that merge exists. Then `F7_APPROVED_SPEC_SHA = C`; verify that `git show C:specs/ARO-FV-1-VISUAL-RELEASE-EVIDENCE.md` is v0.2.4, records the founder-approved 8272CL-only amendment, names PR #48 as the exclusive F7 lane, and retains all §20 thresholds and human gates. C is the approved F7 specification binding, not a replacement for the accepted predecessor. Verify C also retains the v0.2.4 isolated Next input bytes and the §20 fail-closed environment/launch contract; otherwise stop for a new recorded amendment.

`F7_TASK_BASE_SHA` remains the immutable accepted F6 commit `79603ae1af60a30f86c105e0f2a4d841043eb727`. After C exists, resume only existing `codex/fv1-f7-acceptance-evidence` / PR #48. Preserve its F6 preflight evidence, retain F6 as the immutable historical predecessor, and perform the founder-authorized bounded compatibility reconciliation against C’s integrated N1/MERGE1 Next runtime under exact-head review before any browser setup or measurement. Controller ownership clearance recorded at `7526ead5c289229b836bc5e236c9cd9d8c985c3a` (blob `57253a75a7e0fca59a3d1112a6caf5399f43ed52`) confirms the N1 writer inactive; revalidate that concrete state before product compatibility writes, but no additional founder permission is required. Record the exact tested Next execution head separately; do not reapply historical patches, use floating main/HEAD, create a different F7 branch, or treat the Vite F6 tree as the Next runtime.

No F7 controlled functional/performance measurement may begin until C exists, the reviewed PR #48 compatibility head satisfies the §20 isolated Next contract, and the exact lockfile-managed Chromium 151.0.7922.34 / revision 1234 installs in the external Playwright cache with executable SHA-256 and recorded headless launch. This does not permit a system browser, relax a budget, replace the three-run method, or waive independent review, human NVDA/Windows testing, founder visual review, product merge, deployment, promotion or release.
...
Security/privacy/Trust diff reviewer: the independent reviewer above. Approval/findings: **pending; PR #48 contains only F7 preflight evidence and no F7 measurement or final implementation review**.

## 28. Product/design approval record

- Approver: ARO founder/user, explicit approvals in the recovery/implementation task on **2026-09-08**.
- Original approved specification: **FV-1 v0.2.0**, path `specs/ARO-FV-1-VISUAL-RELEASE-EVIDENCE.md`, candidate commit `a576640cc5b9828486d8bdd0f970636b3ff07138`; approved candidate Git blob `e01d450bb41394b13763cf090bdf4fd63d4f8628`. The same candidate content was present in reviewed documentation head `78005892071b7996917f8f10d04e5cd601f124d2`; PR #40 merged as `606d06bcded8e0afde0f2f4eca35a4986650ce5d`.
- Original founder decision: “I approve: 1. Merging PR #40. 2. The proposed FV-1 defaults described in your approval summary. 3. Implementation of F1–F7 sequentially on ONE isolated branch/PR.”
- **v0.2.1 amendment approval:** the prior F1 implementation record demonstrated that CPython 3.13.5 + Pillow 12.3.0 + libwebp 1.6.0 reproduced the exact required 88-byte fingerprint, SHA-256 `a2dd5e25c6a9dd61b8194f496c8b3995a5ba2efc0abc9079ecd2b6cd8ec5450b`, deterministic three-run encoding and exact RGBA/alpha round-trip. The founder approved the narrow encoder-profile change and resumption under the amended §20 conditions. This preserved product behavior, F1–F7 scope, budgets, acceptance thresholds, independent/human review gates and release authority; it does not identify an active F7 execution lane.
- **v0.2.2 execution amendment approval:** the founder instructed “NOW OPTIMIZE AND COMPLETE IT” on 2026-09-09 after F1 measured the required 960px lossless persona at 629,998 bytes against an impossible 400,000-byte cap. This authorizes only the 640,000-byte exception named in section 20. It preserves lossless alpha, source/derivative geometry, every other budget, F1–F7 scope, review gates and withheld release authority.
- **v0.2.3 F7 lab-profile amendment approval:** on 2026-09-14, after the mandatory F7 preflight recorded a matching Ubuntu 24.04.3/x86_64, quota, memory, affinity, Node/npm and lockfile profile but CPU model Intel Xeon Platinum 8272CL rather than 8573C, the founder explicitly approved permitting the 8272CL host for **F7 measurement only**. The founder preserved every other §20 condition, all budgets, Playwright 1.62.0 / Chromium revision 1234, 4× CPU throttling, three-run method, accessibility thresholds, independent review and release WITHHELD. The exact bundled Chromium must still install and launch. This amendment does not weaken any gate.
- **v0.2.4 Next launch/environment amendment approval:** on 2026-09-24, the founder approved the isolated Next.js build-and-runtime contract, normal documentation merge after exact-head checks and independent review, then bounded compatibility reconciliation on existing PR #48, required bundled-browser installation and automated F7 verification. The approval is persisted at controller commit `c145ebd52395a9e8a0104cdaa5ddd78a4672ff35`. It preserves every frozen host/browser/privacy/network/budget/human gate, forbids provider mutation and release, and does not make the Vite F6 base a runnable Next head.
- Documentation: PR #47 is the documentation-only v0.2.4 merge gate. Its actual full main merge commit C becomes `F7_APPROVED_SPEC_SHA` as section 25 defines.
- Implementation: F1–F6 are accepted predecessors; the existing F7-only lane is PR #48 on `codex/fv1-f7-acceptance-evidence` with immutable `F7_TASK_BASE_SHA` F6 `79603ae1af60a30f86c105e0f2a4d841043eb727`. It may perform the reviewed runtime reconciliation only after C; no measurement begins until the exact bundled-browser launch record and every §20 preflight condition exist.
- Release: **WITHHELD**, including product merge and Preview/Production publication. Independent nonwriting review, human NVDA testing and founder visual review remain required before any full-verification claim.
- F7 product acceptance, controlled measurement, independent review, human NVDA testing and founder visual review remain pending. No documentation amendment certifies them.

## 29. Definition of done

VERIFIED requires approved immutable spec, all eight rows passing, scoped tests/required CI, responsive/theme/keyboard/motion/human screen-reader evidence, every budget passing, independent review of the exact final product diff, completed founder visual review, no unrelated changes, and factual existing registry updates. The founder's approvals explicitly withhold any full-verification claim until independent review, human NVDA testing and founder visual review are complete. Product merge/release permissions remain separate and WITHHELD. Do not mark SHIPPED from documentation or CI alone.

## 30. Delivery record

Package FV-1; v0.2.4 is founder-approved but pending normal documentation-only merge through PR #47. The actual merge commit C becomes `F7_APPROVED_SPEC_SHA`; F7's immutable accepted predecessor remains F6 `79603ae1af60a30f86c105e0f2a4d841043eb727`. Existing F7 branch/PR #48 has completed only the bounded preflight: it remains Vite-based, while the reviewed post-C compatibility head must establish the bound Next runtime; exact lockfile-managed Chromium revision 1234 remains absent/unlaunchable. No F7 functional or controlled performance measurement, quality-gate run, independent review, human NVDA test or founder visual review has run. All thresholds and release gates remain WITHHELD. Recovery provenance remains in [RECOVERY.md](../docs/fv1-recovery/RECOVERY.md).
