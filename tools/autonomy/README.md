...
Route inventory supports both the historical literal routes in `src/lib/routes.jsx` and committed Next.js App Router pages matching `src/app/**/page.{js,jsx,ts,tsx}`. Next.js route groups are omitted from declared URLs, private folders are excluded, and dynamic/catch-all segment notation is preserved. Mixed legacy/App Router inputs and unsupported parallel or intercepting segments fail closed. This is source inventory only; it does not execute redirects, rewrites, middleware or route handlers and is not browser acceptance.
