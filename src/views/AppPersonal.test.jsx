import { startProductionServer } from '../test/production-server'
...
async function startF5BrowserServer() { return startProductionServer(4181) }
...
describe('FV-1 F5 browser acceptance evidence', () => {
  browserEvidenceIt('verifies production Profile/Express responsive, zoom, keyboard, focus, contrast, media and local-only behavior', async () => {
    ...
    try {
      ...
      for (const theme of themes) {
        ...
        for (const width of widths) {
          ...
          for (const routeCase of routeCases) {
            await page.goto(`${base}${routeCase.route}`, { waitUntil: 'networkidle' })
      ...
      interactionPage.on('request', (request) => { if (['fetch', 'xhr'].includes(request.resourceType()) && !(request.method() === 'GET' && new URL(request.url()).origin === base && request.headers().rsc === '1')) serviceRequests.push(request.url()) })

      await interactionPage.goto(`${base}/app/profile`, { waitUntil: 'networkidle' })
      ...
      await interactionPage.goto(`${base}/app/express`, { waitUntil: 'networkidle' })
      ...
      await reducedPage.goto(`${base}/app/profile`, { waitUntil: 'networkidle' })
      ...
      await reducedPage.goto(`${base}/app/express`, { waitUntil: 'networkidle' })
