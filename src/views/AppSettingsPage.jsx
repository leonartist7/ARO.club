...
export default function AppSettingsPage() {
  ...
  return (
    <div lang={language} className="px-4 py-6 sm:px-8 sm:py-10">
      <Link to="/app/profile" className="inline-flex min-h-11 items-center gap-2 text-base font-bold text-ink/70 focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-primary-500 dark:text-bone/75"><ArrowLeft className="h-4 w-4" aria-hidden="true" /> {copy.settings.back}</Link>
        ...
        {copy.settings.rows.map((row) => {
          ...
          return (
            <article key={row.id} data-fv1-setting-row={row.id} className="flex min-h-[92px] min-w-0 flex-wrap items-center gap-4 px-5 py-4 sm:flex-nowrap sm:px-6">
              ...
              <div className="min-w-[min(10rem,100%)] flex-1"><h2 className="text-base font-bold">{row.label}</h2><p data-fv1-essential-copy className="mt-1 text-base leading-6 text-ink/70 dark:text-bone/75">{row.detail}</p></div>
              <span className="w-full min-w-0 break-words text-left text-sm font-bold text-ink/70 dark:text-bone/75 sm:w-auto sm:max-w-[12rem] sm:shrink-0 sm:text-right">{copy.common.unavailable}</span>
            </article>
          );
        })}
      </AppPanel>
