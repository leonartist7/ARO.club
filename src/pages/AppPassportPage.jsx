      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(280px,0.7fr)]">
        <AppPanel className="overflow-hidden">
          <div className="relative min-h-[320px] overflow-hidden bg-ink p-6 text-bone aro-grid dark:bg-plum sm:p-8">
            <AppImage src="/aro-passport-life-map-v1.png" alt={copy.passport.heroAlt} variant="hero" cropClass="object-[69%_center]" className="absolute inset-0 h-full w-full object-cover" />
          ...
          <div className="grid grid-cols-1 border-t border-ink/10 dark:border-bone/10 sm:grid-cols-3">{copy.passport.stats.map((stat, index) => <div key={stat.label} className={`p-5 ${index ? 'border-t border-ink/10 dark:border-bone/10 sm:border-l sm:border-t-0' : ''}`}><p className="font-display text-3xl">{stat.value}</p><p data-fv1-essential-copy className="mt-1 text-base leading-6 text-ink/70 dark:text-bone/75">{stat.label}</p></div>)}</div>
        </AppPanel>
        <div className="space-y-6"><AppPanel className="p-6"><HeartHandshake className="h-6 w-6 text-primary-500" aria-hidden="true" /><h2 className="mt-5 font-display text-3xl">{copy.passport.contributionTitle}</h2><p data-fv1-essential-copy className="mt-3 text-base leading-6 text-ink/70 dark:text-bone/75">{copy.passport.contributionBody}</p></AppPanel><AppPanel className="p-6"><Compass className="h-6 w-6 text-sky" aria-hidden="true" /><h2 className="mt-5 font-display text-3xl">{copy.passport.intentionTitle}</h2><p data-fv1-essential-copy className="mt-3 text-base leading-6 text-ink/70 dark:text-bone/75">{copy.passport.intentionBody}</p></AppPanel></div>
      </div>

      <section className="mt-14"><AppSectionHeading eyebrow={copy.passport.recentEyebrow} title={copy.passport.recentTitle}><span className="text-base font-semibold text-ink/70 dark:text-bone/75">{copy.common.previewOnly}</span></AppSectionHeading><div className="mt-5 divide-y divide-ink/10 border-y border-ink/10 dark:divide-bone/10 dark:border-bone/10">{passportEntries.map((entry) => <article key={entry.title} data-fv1-passport-entry className="grid gap-4 py-5 sm:grid-cols-[80px_92px_1fr_auto] sm:items-center"><div className="flex items-center gap-2 sm:block"><span className="text-sm font-bold tracking-[0.14em] text-primary-600 dark:text-primary-300">{entry.month}</span><span className="font-display text-3xl">{entry.day}</span></div><div aria-hidden="true" className="h-20 w-full overflow-hidden rounded-2xl sm:w-[92px]"><AppImage src={entry.image} alt="" variant="thumbnail" cropClass={entry.imagePosition} className="h-full w-full object-cover" /></div><div><p className="text-sm font-bold uppercase tracking-[0.14em] text-ink/70 dark:text-bone/75">{copy.common.example}</p><h2 className="mt-1 font-display text-2xl">{entry.title}</h2><p data-fv1-essential-copy className="mt-1 text-base leading-6 text-ink/70 dark:text-bone/75">{entry.meta}</p></div><span className="inline-flex items-center gap-2 text-sm font-bold text-ink/70 dark:text-bone/75"><Sparkles className="h-4 w-4 text-secondary-500" aria-hidden="true" /> {copy.passport.recentStatus}</span></article>)}</div></section>
    </div>
  );
}