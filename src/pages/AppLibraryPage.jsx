    ...
    <div lang={language} className="px-4 py-6 sm:px-8 sm:py-10">
      ...
      <section className="mt-6 border-b border-ink/10 pb-4 dark:border-bone/10" aria-labelledby="library-filter-label">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {copy.library.filters.map((filter) => <span key={filter} data-fv1-library-filter className="border border-ink/10 bg-white/60 px-3 py-2 text-sm font-semibold text-ink/70 dark:border-bone/10 dark:bg-gray-900/50 dark:text-bone/75">{filter}</span>)}
        </div>
        <p data-fv1-essential-copy className="mt-3 text-base leading-6 text-ink/70 dark:text-bone/75">{copy.library.filterNote}</p>
      </section>
...
function LibraryRow({ row, item, copy }) {
  ...
  const content = (
    <AppPanel className={`flex min-w-0 flex-wrap items-center gap-4 p-3 sm:flex-nowrap sm:p-4 ${row.href ? 'transition-colors group-hover:border-primary-500/50' : 'bg-white/45 dark:bg-gray-900/45'}`}>
      {row.image ? (
        <div aria-hidden="true" className="h-16 w-16 shrink-0 overflow-hidden bg-ink sm:h-20 sm:w-20"><AppImage src={row.image} alt="" variant="thumbnail" className="h-full w-full object-cover" /></div>
      ) : (
      ...
      <div className="min-w-[10rem] flex-1">
        ...
        <p data-fv1-essential-copy className="mt-1 text-base leading-6 text-ink/70 dark:text-bone/75">{item.meta}</p>
      </div>
      <div className="w-full min-w-0 text-left sm:w-auto sm:max-w-[13rem] sm:shrink-0 sm:text-right">
        <p className={`break-words text-sm font-bold ${row.href ? 'text-primary-700 dark:text-primary-300' : 'text-ink/70 dark:text-bone/75'}`}>{row.href ? copy.library.openExample : copy.common.unavailable}</p>
      </div>
    </AppPanel>
  );
