...
function PersonalField({ copy, activeNodeId, onSelect }) {
  ...
  return (
    <section data-fv1-personal-field className="relative isolate overflow-hidden rounded-[2.25rem] bg-ink px-5 py-6 text-bone shadow-[0_28px_80px_rgba(40,36,32,0.24)] sm:px-8 sm:py-8">
      ...
      <div data-fv1-profile-stage className="relative z-10 mt-8 rounded-[1.75rem] border border-bone/10 bg-bone/[0.04] p-4 sm:p-6">
  ...
  return (
    <div lang={language} className="mx-auto max-w-[1180px] px-4 py-6 sm:px-8 sm:py-9">
      ...
      <Link to="/app/express" className="group mt-6 flex flex-col gap-5 overflow-hidden border border-ink/10 bg-primary-50/70 p-5 transition hover:border-primary-500/45 hover:bg-primary-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 dark:border-bone/10 dark:bg-primary-900/15 dark:hover:bg-primary-900/25 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div><p className="flex items-center gap-2 text-sm font-bold uppercase tracking-[0.16em] text-primary-700 dark:text-primary-300"><Sparkles className="h-4 w-4" aria-hidden="true" /> {copy.profile.expressEyebrow}</p><h2 className="mt-3 font-display text-3xl leading-[0.96]">{copy.profile.expressTitle}</h2><p className="mt-3 max-w-xl text-base leading-6 text-ink/70 dark:text-bone/75">{copy.profile.expressBody}</p></div><span className="inline-flex min-h-11 items-center gap-2 self-start whitespace-nowrap text-sm font-bold text-primary-700 transition group-hover:text-primary-500 dark:text-primary-300 sm:self-auto">{copy.profile.expressLink} <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" /></span>
      </Link>
