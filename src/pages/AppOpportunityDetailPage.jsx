...
function MissingExample({ copy }) {
  return (
    <section className="mx-auto flex min-h-[50vh] max-w-xl flex-col justify-center px-4 py-12 sm:px-8" aria-labelledby="journey-missing-title">
      <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary-600 dark:text-primary-300">{copy.fictionalLabel}</p>
      <h1 id="journey-missing-title" className="mt-3 font-display text-4xl text-ink dark:text-bone">{copy.missingTitle}</h1>
...
export default function AppOpportunityDetailPage() {
  ...
  const count = opportunity.exampleCount;
  const status = getFv1FormationStatus(copy, opportunity);
  const minimumProgress = Math.min((count / opportunity.minimum) * 100, 100);

  return (
    <div className="mx-auto max-w-[1180px] px-4 py-6 sm:px-8 sm:py-9">
      <Link to="/app/world" className="inline-flex min-h-11 items-center gap-2 text-sm font-bold text-ink/55 transition hover:text-ink dark:text-bone/55 dark:hover:text-bone"><ArrowLeft className="h-4 w-4" aria-hidden="true" /> {copy.backToWorld}</Link>
      ...
      <section className="mt-5 overflow-hidden rounded-[2rem] bg-ink text-bone shadow-[0_22px_60px_rgba(40,36,32,0.22)]">
        <div className="relative min-h-[430px] sm:min-h-[510px]">
          ...
          <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-4 p-5 sm:p-7">
            <div className="rounded-full border border-white/20 bg-ink/45 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.16em] text-secondary-100 backdrop-blur-xl"><span className="mr-2 inline-block h-1.5 w-1.5 rounded-full bg-secondary-300" />{copy.fictionalLabel}</div>
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 sm:p-8">
            <div className="max-w-3xl"><div className="flex flex-wrap items-center gap-2"><StatusPill tone={opportunity.status}>{copy.fictionalLabel}</StatusPill><span className="text-[11px] font-bold uppercase tracking-[0.16em] text-bone/65">{opportunity.area} · {opportunity.distance}</span></div><h1 className="mt-4 font-display text-4xl leading-[0.9] tracking-[-0.035em] sm:text-6xl">{opportunity.title}</h1><p className="mt-4 max-w-xl text-base leading-7 text-bone/75 sm:text-lg">{opportunity.summary}</p></div>
          </div>
        </div>
      </section>
      ...
      <div className="mt-7 grid gap-7 lg:grid-cols-[minmax(0,1fr)_350px] lg:items-start">
        <div className="space-y-7">
          <section className="grid gap-4 border-y border-ink/10 py-5 dark:border-bone/10 sm:grid-cols-3">
            <span className="flex items-start gap-3"><MapPin className="mt-0.5 h-5 w-5 text-primary-500" aria-hidden="true" /><span><strong className="block text-sm">{opportunity.place}</strong><span className="text-xs text-ink/55 dark:text-bone/55">{copy.examplePlaceOnly}</span></span></span>
            ...
            <span className="flex items-start gap-3"><UsersRound className="mt-0.5 h-5 w-5 text-moss" aria-hidden="true" /><span><strong className="block text-sm">{copy.examplePlaces(count, opportunity.capacity)}</strong><span className="text-xs text-ink/55 dark:text-bone/55">{copy.exampleMinimum(opportunity.minimum)}</span></span></span>
          </section>
